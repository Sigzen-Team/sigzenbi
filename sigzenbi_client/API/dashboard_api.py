...
@frappe.whitelist(allow_guest=True)
def get_analytics_token(dashboard_id=None):
    """
    Proxy request to central server to generate an analytics guest token for the client user.
    """
    try:
        central_sid = None
        if getattr(frappe.local, "request", None):
        ...
        try:
            ...
            frappe.log_error(title="get_analytics_token: RLS computation", message=frappe.get_traceback())

        # This tenant's REAL portal origin (any customer domain), derived server-side from the
        # browser's own request — never hardcoded, never from config. Central uses it to authorize
        # this tenant's analytics embeds for THIS domain (SaaS per-tenant embed authorization).
        # Prefer the Origin header (exactly what the analytics allowed_domains list checks); fall back to
        # scheme://host. Central re-validates before trusting it.
        embed_origin = None
        if getattr(frappe.local, "request", None):
            try:
                embed_origin = frappe.get_request_header("Origin")
                if not embed_origin:
                    scheme = frappe.get_request_header("X-Forwarded-Proto") or frappe.request.scheme or "https"
                    if frappe.request.host:
                        embed_origin = f"{scheme}://{frappe.request.host}"
            except Exception:
                embed_origin = None
        ...
        frappe.log_error(title="get_analytics_token_client", message=f"Error in client get_analytics_token: {str(e)}")
        return {"success": False, "message": str(e)}
