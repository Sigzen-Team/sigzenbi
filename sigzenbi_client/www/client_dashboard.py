...
def central_get_with_sid(url, sid, timeout=10):
	...
	Team and Billing links from their sidebar (2026-08-02): can_manage_analytics_login
...
def get_context(context):
    ...
    # The client site has no Client User doctype, so the "Analytics login" card gate is
    ...
    context.can_manage_analytics_login = 0
    # Analyst-only, and a SEPARATE question from can_manage_analytics_login (which also gates the
    ...
    if base_url and central_sid:
        ...
        if _r is not None and _r.ok:
            ...
            context.can_manage_analytics_login = _flags.get("can_manage", 0)
    ...
    context.api_get_analytics_token_url = "sigzenbi_client.API.dashboard_api.get_analytics_token"
    ...
    if not central_html:
        ...
        try:
            ...
            central_html = central_html.replace(
                ...
                context.api_get_analytics_token_url
            )
            central_html = central_html.replace(
            ...
            central_html = central_html.replace(
                ...
                "sigzenbi_client.API.team_proxy.get_my_analytics_password"
            )
            central_html = central_html.replace(
                "sigzenbi_central.API.team.superset_credentials.reset_superset_password",
                "sigzenbi_client.API.team_proxy.reset_analytics_password"
            )
            central_html = central_html.replace(
