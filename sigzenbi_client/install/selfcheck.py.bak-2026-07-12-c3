...
def _check_central_reachable():
    """Authenticated ping: get_active_client_names, gated by the gateway shared
    secret. Deliberately NOT one of the flagged allow_guest debug endpoints
    (test_ping_v5 etc.) — this one requires a valid secret to answer."""
    central_url = _central_url()
    secret = frappe.conf.get("sigzen_gateway_shared_secret")
    if not central_url or not secret:
        return False, "central_url or gateway secret not configured"
    try:
        resp = requests.get(
