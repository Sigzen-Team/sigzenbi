...
# The SigzenBI Central hub + shared analytics URLs. These are PRODUCT constants (the one
...
# deployment overrides these via site_config keys (sigzenbi_central_url / sigzenbi_analytics_url)
# or, post-install, via install_agent.sh --central-url (register_agent.run).
DEFAULT_CENTRAL_URL = "https://central.sigzen.com"
DEFAULT_ANALYTICS_URL = "https://app.sigzenbi.com"
...
def _analytics_url():
    return (frappe.conf.get("sigzenbi_analytics_url") or DEFAULT_ANALYTICS_URL).rstrip("/")
...
def set_default_subscription_settings():
    ...
    current_analytics = frappe.db.get_single_value("SigzenBI Subscription Settings", "sigzenbi_link")
    updates = {}
    if not (current_central or "").strip():
        updates["sigzenbi_erp_link"] = _central_url()
    if not (current_analytics or "").strip():
        updates["sigzenbi_link"] = _analytics_url()
    if updates:
